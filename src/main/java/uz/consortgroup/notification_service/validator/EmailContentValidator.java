...
public class EmailContentValidator {
    ...
    public boolean isUserProfileUpdatedEvent(EventType type) {
        boolean isUpdated = type == EventType.USER_PROFILE_UPDATED;
        log.debug("Checking if event is USER_PROFILE_UPDATED: {}", isUpdated);
        return isUpdated;
    }

    public void validateBuilder(EmailContent content, EmailMessageBuilder builder, EventType type) {
        if (builder == null) {
            log.warn("No EmailMessageBuilder found for event type: {}", type);

            if (content == null || content.getEmail() == null) {
                log.error("Email content or email address is null");
